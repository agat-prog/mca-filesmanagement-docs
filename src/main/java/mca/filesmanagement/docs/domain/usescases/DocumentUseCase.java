...
public class DocumentUseCase implements IDocumentUseCase {
	...
	@Override
	public long createDocument(DocumentNewDto documentDto) throws DocumentCreationException {
		try {
			long id = this.documentRepository.createDocument(documentDto);
			DocumentDto docSave = this.documentRepository.getById(id);			
			String URL = this.documentBinaryRepositorio.createOrUpdate(docSave.getCode(), Base64Utils.decodeFromString(documentDto.getContentBase64()));
